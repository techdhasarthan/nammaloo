...
export default function ProfileScreen() {
  ...
  const loadUserData = async () => {
    try {
      ...
      if (user) {
        const savedToilets = await userService.getSavedToilets(user._id);
        setSavedToiletsCount(savedToilets.length);
      }
    } catch (error) {
            ...
            <View style={styles.savedToiletsContent}>
              ...
              <View style={styles.savedToiletsPlaceholder}>
                ...
                <Text style={styles.placeholderText}>
                  {savedToiletsCount === 0 ? 'No saved toilets yet' : `${savedToiletsCount} toilets saved`}
                </Text>
