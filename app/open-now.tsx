...
export default function OpenNowPage() {
  ...
  const [distanceCalculationStatus, setDistanceCalculationStatus] = useState('');
  ...
  const loadToilets = async () => {
    try {
      ...
      // Get open toilets using the new service
      const filters = {
        openNow: true,
        ...(userLocation && {
  ...
  // Get distance using location utils
  ...
  const getTimeColor = (workingHours) => {
  ...
  const getOpenStatus = (toilet) => {
    if (!toilet.workingHours) return 'Open 24/7';
    if (toilet.workingHours.includes('24')) return 'Open 24/7';
    return workingHoursUtils.formatWorkingHours(toilet.workingHours);
  };
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.resultsHeader}>
        ...
        <Text style={styles.resultsSubtext}>
          Real-time availability • {userLocation ? 'With distance calculations' : 'Enable location for distances'}
        </Text>
        {distanceCalculationStatus && (
          <Text style={styles.distanceStatusText}>{distanceCalculationStatus}</Text>
        )}
        ...
        {toilets.length === 0 ? (
          ...
          toilets.map((toilet, index) => (
            ...
            >
              ...
              <Image 
                source={{ 
                  uri: toilet.imageUrl || 'https://images.pexels.com/photos/6585756/pexels-photo-6585756.jpeg?auto=compress&cs=tinysrgb&w=400'
                }} 
              ...
              <View style={styles.cardContent}>
                ...
                <View style={styles.ratingRow}>
                  <View style={styles.ratingContainer}>
                    ...
                    <Text style={styles.reviewCount}>({toilet.reviewCount || 0} reviews)</Text>
                  </View>
                  
                  <View style={styles.statusContainer}>
                    <View style={[styles.statusDot, { backgroundColor: workingHoursUtils.getStatusColor(toilet.workingHours) }]} />
                    <Text style={[styles.statusText, { color: workingHoursUtils.getStatusColor(toilet.workingHours) }]}>
                      {workingHoursUtils.getStatusText(toilet.workingHours)}
                    </Text>
                  </View>
                </View>
